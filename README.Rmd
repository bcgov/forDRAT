...
You can install `forDRAT` directly from this GitHub repository.
...
Next, install and load the `forDRAT` package using `remotes::install_github()`:
...
1. *drought_data* data frame which contains estimated mean annual AET:PET for xeric to subhygric sites in 210 BGC variants, for four different time periods.
2. *tree_risk* data frame which contains estimated estimated AET:PET thresholds for different tree species, based on distribution of trees species across actual soil moisture regimes, and field observations.  
